...
/**
 * Demo of Foreign Memory Access API after, 7 or 8? previews,
 * based on https://openjdk.org/jeps/454
 */
...
//            intElemHandle.set(segment, 0L, (long) i, i);
        }
    }

    private static void examineNativeMem(SequenceLayout intArrayLayout, MemorySegment segment) {
        System.out.println(STR."Examining memory at: \{segment.address()}");
        System.out.println(STR."Is accessible by current thread? \{
            segment.isAccessibleBy(Thread.currentThread())}");
        VarHandle intElemHandle = intArrayLayout.varHandle(MemoryLayout.PathElement.sequenceElement());
        for (int i = 0; i < intArrayLayout.elementCount(); i++) {
            System.out.print(segment.getAtIndex(ValueLayout.JAVA_INT, i));
            System.out.print(" ");
            System.out.println(intElemHandle.get(segment, 0L, i));
        }
    }
}
